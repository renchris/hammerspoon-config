...
pathWatcher = hs.pathwatcher.new(os.getenv("HOME") .. "/Library/Preferences", function(files)
  for _, f in ipairs(files) do
    if f:match("com%.apple%.dock%.plist$") then
      hs.timer.doAfter(0.5, rebind) -- slight delay so macOS finishes writing
      break
    end
  end
end)
...
-- Manual refresh: ⌥+⌘+R
hs.hotkey.bind({ "alt", "cmd" }, "R", rebind)
...
local smartPasteTap
smartPasteTap = hs.eventtap.new({hs.eventtap.event.types.keyDown, hs.eventtap.event.types.keyUp}, function(event)
    local flags = event:getFlags()
    local keyCode = event:getKeyCode()
    local isDown = event:getType() == hs.eventtap.event.types.keyDown

    -- Only intercept Cmd+V keyDown in terminal apps with image on clipboard
    if keyCode ~= 9 or not isDown then return false end
...
local function dismissThumbnail()
    ...
    local step = 0
    hs.timer.doEvery(THUMB_FADE / steps, function()
        step = step + 1
        if step >= steps then
            c:delete()
            return
        end
        c:alpha(1 - step / steps)
    end)
end
...
local function showThumbnail(path, img)
    ...
    thumbCanvas:mouseCallback(function(_, _, id, x, y)
        if id == "mouseUp" then
            hs.execute('open -a Preview "' .. path .. '"')
            dismissThumbnail()
        end
    end)
    thumbCanvas:canvasMouseEvents(true)
    ...
    thumbSlideTimer = hs.timer.doEvery(THUMB_SLIDE_DUR / slideSteps, function()
        ...
        if slideStep >= slideSteps then
            thumbCanvas:topLeft({ x = finalX, y = finalY })
            if thumbSlideTimer then thumbSlideTimer:stop(); thumbSlideTimer = nil end
            return
        end
        local t = slideStep / slideSteps
        local ease = 1 - (1 - t) ^ 3
        thumbCanvas:topLeft({ x = startX - dist * ease, y = finalY })
    end)
...
local function processScreenshot(path)
    ...
    -- Copy to clipboard: TIFF (native paste) + PNG (web file inputs)
    local pngFile = io.open(path, "rb")
    if not pngFile then return end
    local pngData = pngFile:read("*a")
    pngFile:close()

    img:saveToFile("/tmp/hs_screenshot.tiff", "tiff")
    local tiffFile = io.open("/tmp/hs_screenshot.tiff", "rb")
    if not tiffFile then return end
    local tiffData = tiffFile:read("*a")
    tiffFile:close()
    os.remove("/tmp/hs_screenshot.tiff")

    hs.pasteboard.clearContents()
    hs.pasteboard.writeAllData({
        ["public.png"] = pngData,
        ["public.tiff"] = tiffData,
    })
    ...
    for _, path in ipairs(files) do
        if path:match("Screenshot .+%.png$") then
            ...
            if attrs and attrs.mode == "file" then
                hs.timer.doAfter(0.1, function() processScreenshot(path) end)
            end
        end
    end
end)
